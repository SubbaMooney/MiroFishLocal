...
import { renderMarkdown, sanitizeHtml, escapeAndFormatBoldNewline } from '../utils/markdown'
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  h('div', {
                    ...
                    // Fix H1: Sowohl der Placeholder-Text (statisch i18n)
                    // als auch der LLM-Antworttext werden über
                    // sanitizeHtml geschickt. Das Markdown-ähnliche
                    // Bold + <br>-Mapping läuft auf escapedem Inhalt
                    // (renderMarkdown würde hier zu viel tun, deshalb
                    // ein minimales Inline-Escape vorab + Sanitize
                    // hinten).
                    innerHTML: sanitizeHtml(
                      isPlaceholder
                        ? answerText
                        : escapeAndFormatBoldNewline(formatAnswer(answerText, isExpanded))
                    )
                  }),
...
// renderMarkdown wird aus '../utils/markdown' importiert (Fix H1).
// Der Renderer escaped die Eingabe und sanitisiert die Ausgabe per
// DOMPurify, damit LLM-/Persona-Output (Reports, Quotes, Summaries)
// keine ausführbaren Skript-Tags mehr ins DOM injizieren kann.
