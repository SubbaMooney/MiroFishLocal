...
// Polling timer — Master-Tick (1s) statt drei separater Intervalle.
// Die "*Timer"-Flags repraesentieren weiterhin "Polling fuer Endpoint X aktiv?".
let pollTimer = null      // Flag: pollPrepareStatus aktiv (Intervall 2s)
let profilesTimer = null  // Flag: fetchProfilesRealtime aktiv (Intervall 3s)
let configTimer = null    // Flag: fetchConfigRealtime aktiv (Intervall 2s)
let masterTimer = null    // Tatsaechlicher setInterval-Handle (1s)
let masterTick = 0        // Tick-Counter, +1 pro Sekunde

// Master-Tick startet bei Bedarf; pro Tick werden alle aktivierten Endpoints
// parallel via Promise.allSettled abgefragt, damit ein einzelner Fehler die
// anderen nicht blockt.
...
const stopMasterTimerIfIdle = () => {
  if (!pollTimer && !profilesTimer && !configTimer && masterTimer) {
...
const runMasterTick = async () => {
  masterTick += 1
  const due = []
  // 2s Intervall fuer pollPrepareStatus
  if (pollTimer && masterTick % 2 === 0) due.push(pollPrepareStatus())
  // 3s Intervall fuer fetchProfilesRealtime
  if (profilesTimer && masterTick % 3 === 0) due.push(fetchProfilesRealtime())
  // 2s Intervall fuer fetchConfigRealtime
  if (configTimer && masterTick % 2 === 0) due.push(fetchConfigRealtime())
  if (due.length > 0) {
    // Fehler einzelner Calls duerfen die anderen nicht stoppen.
...
onUnmounted(() => {
  // Flags abschalten — letzter stop killt den Master-Timer via stopMasterTimerIfIdle.
  stopPolling()
  stopProfilesPolling()
  stopConfigPolling()
  // Defensive: falls aus irgendeinem Grund noch ein Master-Timer laeuft, killen.
  if (masterTimer) {
    clearInterval(masterTimer)
    masterTimer = null
    masterTick = 0
